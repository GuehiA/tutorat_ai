...
@app.route("/create-persistent-admin")
def create_persistent_admin():
    """Crée un admin PERSISTANT - les données ne seront plus perdues"""
    try:
        with app.app_context():
            from datetime import datetime
            ...
            if admin:
                # Admin existe, vérifier/mettre à jour le mot de passe
                if admin.verifier_mot_de_passe(password):
                    status = "✅ Admin existe déjà avec le bon mot de passe"
                else:
                    admin.mot_de_passe = password
                    status = "✅ Mot de passe admin mis à jour"
            else:
                # Créer nouvel admin
                admin = User(
                    username="admin",
                ...
                status = "✅ Nouvel admin créé"
            
            # S'assurer que c'est bien un admin payé (pas en essai)
            admin.statut_essai = "payant"
            admin.statut_paiement = "paye"
            
            db.session.commit()
            
            # Tester la connexion
            test = admin.verifier_mot_de_passe(password)
            
            return f"""
            <h1>🎉 Admin PERSISTANT créé !</h1>
            <p><strong>Status:</strong> {status}</p>
            <p><strong>Email:</strong> {email}</p>
            <p><strong>Mot de passe:</strong> {password}</p>
            <p><strong>Test de connexion:</strong> {"✅ RÉUSSI" if test else "❌ ÉCHEC"}</p>
            <p><strong>Role:</strong> {admin.role}</p>
            <p><strong>Statut paiement:</strong> {admin.statut_paiement}</p>
            <hr>
            <p><strong>⚠️ IMPORTANT :</strong></p>
            <p>1. Supprimez cette route après usage</p>
            <p>2. Supprimez aussi <code>@app.before_first_request</code> dans app.py</p>
            <p>3. Supprimez <code>db.drop_all()</code> dans seed.py</p>
            <p><strong>Sinon les données seront perdues au prochain déploiement !</strong></p>
            <hr>
            <a href="/login-admin" style="display: inline-block; padding: 12px 24px; background: #4361ee; color: white; text-decoration: none; border-radius: 8px; margin-top: 20px;">
